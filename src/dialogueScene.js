...
export default class DialogueScene extends Phaser.Scene {
  ...
  preload() {
    this.load.image("bg-dialogue", "img/bg-dialogue.png");
    this.load.image("pointer", "img/pointer.png");
    this.load.image("npc-0", "img/npc-0.png");
    this.load.image("npc-1", "img/npc-1.png");
    this.load.image("npc-2", "img/npc-2.png");
    this.load.image("npc-3", "img/npc-3.png");
    this.load.image("npc-4", "img/npc-4.png");
    this.load.image("npc-5", "img/npc-5.png");
    this.load.image("npc-6", "img/npc-6.png");
    this.load.audio("npc-1", "audio/npc-1.mp3");
    this.load.audio("npc-2", "audio/npc-2.mp3");
    this.load.audio("npc-3", "audio/npc-3.mp3");
    this.load.audio("npc-4", "audio/npc-4.mp3");
    this.load.audio("npc-5", "audio/npc-5.mp3");
    this.load.audio("npc-6", "audio/npc-6.mp3");
  }
  ...
  async create() {
    ...
    this.dialogues = [
      [
        ...
        "First, check if the book's overdue and charge a late fee if it's past the due date. You can check today's date using the calendar on the right side of your desk.",
        "Next you want to open the book and use the tools on the damaged book to repair it. Use your handy reference card on the bottom left to figure out which tool is appropriate for the damage.",
        "Be careful! If you take too long to repair it, the book will become unstable and summon a dangerous monster!",
        "Good luck!"
      ],
      [
    ...
    setState("spellCorrectedChars", 0);
    setState("spellCompleted", false);
  }
  ...
  advanceDialogue() {
    ...
    if (this.currentDialogue < this.dialogues[level].length) {
      if (this.currentDialogue === 1 && level !== 0) {
        this.sound.play(`npc-${level}`);
      }
      this.displayDialogue(this.dialogues[level][this.currentDialogue]);
